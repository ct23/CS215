...
public class Tests {
	...
	private void mergeTests(int numTests) {
		...
		for (int i = 0; i < numTests; i++) {
			// Sort each array
			long avgSortTime = mSorter.mergeSortDriver(usableOArray, p, r);
			long bestSortTime = mSorter.mergeSortDriver(usableSArray, p, r);
			long worstSortTime = mSorter.mergeSortDriver(usableRArray, p, r);
